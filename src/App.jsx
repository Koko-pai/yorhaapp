  ...
  return (
    <div style={{ position:"fixed", inset:0, zIndex:9996, background:"rgba(0,0,0,0.92)", display:"flex", alignItems:"center", justifyContent:"center", fontFamily:"'Courier New',monospace", padding:16 }}
      ...
      <div style={{ background:"#0a0a0a", border:"1px solid #222", borderTop:"2px solid "+accent, maxWidth:420, width:"100%", maxHeight:"85vh", overflowY:"auto", padding:24, position:"relative" }}
        ...
        {[
          { title:"◆ МИССИИ", body:"Запрашивай задания у командования. Система выдаёт миссии из банка директив с учётом экипированного оружия. Лимит: 3 генерации в день (сброс в полночь). Каждое нажатие кнопки даёт 3 миссии.\n\nТипы миссий:\n· Обычная (65%) — НИЗКАЯ → 20-30 MEM + 1◈, СРЕДНЯЯ → 40-55 MEM + 2-3◈, ВЫСОКАЯ → 65-80 MEM + 4-5◈\n· ⚠ ЭВЕНТ (15%) — срочная директива, награда ×2 от обычной\n· ЛОГИКА (20%) — интеллектуальная задача, всегда СРЕДНЯЯ сложность, 65-88 MEM + 4-5◈\n\nКнопка ↺ на карточке перегенерирует миссию — доступно 3 раза, потом блокировка на 1 час." },
          ...
          { title:"◈ ЮНИТ", body:"Вкладка ЮНИТ — статус и коллекция форм. Три формы: SENTINEL SAVIOR (базовая), ABSTRACT SAVIOR (v5.0), REBORN WARDEN (v9.0). Кликни на разблокированную форму чтобы сменить активную. Здесь же — 28 достижений: выполняй миссии, сражайся, собирай предметы чтобы разблокировать их." },
          { title:"⚔ ОРУЖИЕ", body:"Влияет на стиль генерируемых миссий. Редкое оружие даёт бонус +10-30% к Памяти за совместимые задания. Получается через гачу — надеть можно во вкладке БРОНЯ." },
          { title:"✦ АРХИВ (ГАЧА)", body:"Накопи 10 ◈ фрагментов и извлеки запись из архива. Можно получить: титулы (отображаются под именем), цветовые схемы (меняют весь интерфейс), оружие (влияет на миссии), лор-файлы (цитаты из вселенной NieR), снаряжение (влияет на боевые показатели). Дубликаты снаряжения — каждый экземпляр уникален со своими характеристиками. Дубликаты остальных предметов конвертируются в ◈ фрагменты." },
          { title:"🛡 БРОНЯ (СНАРЯЖЕНИЕ)", body:"Вкладка БРОНЯ — надевай и улучшай снаряжение: шлем, нагрудник, перчатки, поножи, оружие. Каждый предмет имеет основной и дополнительные статы (АТК, HP, КРИТ.ШНС, КРИТ.УРОН). Сеты снаряжения дают бонусы при комплектовании 2 или 4 предметов одного набора. Улучшение требует материалов, добытых в боевом режиме." },
          { title:"⚡ БОЕВОЙ РЕЖИМ", body:"Вкладка БОЙ — сражайся с машинами в волновом бою. Используй способности: POD FIRE (урон всем), EVADE (неуязвимость + атака), BLADE STORM (200% урон, гарантированный крит при HP < 30%). Каждая волна приносит материалы для улучшения брони. Ежедневно обновляются 3 боевые миссии с наградами MEM и ◈. Боссы появляются каждые 10 волн — за них выпадает редкий лут." },
          ...
          { title:"◈ РЕДКОСТЬ", body:"Обычный (60%) · Редкий (25%) · Эпический (12%) · Легендарный (3%). Дубликаты снаряжения не теряются — каждый экземпляр уникален. Дубликаты остальных предметов конвертируются во фрагменты: Обычный → 5◈, Редкий → 10◈, Эпический → 15◈, Легендарный → 20◈." },
